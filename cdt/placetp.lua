if game.PlaceId == 1554960397 then
    ...
    local servers = {}
    local req = game:HttpGet("https://games.roblox.com/v1/games/135202704953082/servers/Public?sortOrder=Desc&limit=100&excludeFullGames=true")
    local body = HttpService:JSONDecode(req)
    
    if body and body.data then
        for i, v in next, body.data do
            if type(v) == "table" and tonumber(v.playing) and tonumber(v.maxPlayers) then
                if v.playing < 50 and v.playing < v.maxPlayers and v.id ~= JobId then
